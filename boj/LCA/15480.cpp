...
int main(){
	...
	parent[1][0] = 1; 
	DFS(1,0);  // 어느 정점 기준 Tree 이라도 상관 없다.
	find_pow_parent(node_num);
	scanf("%d",&query_num);
	int ans, max_depth,tmp;
	for(int i = 0;i<query_num;i++){
		scanf("%d %d %d",&a,&b,&c); 
		// LCA(a,b),LCA(b,c),LCA(a,c) 중에서 가장 depth 가 깊은 LCA가 정답
		tmp = LCA(a,b);
		max_depth = depth[tmp];
		ans = tmp;
		tmp = LCA(b,c);
		if(depth[tmp]>max_depth){
			max_depth = depth[tmp];
			ans = tmp;
		}
		tmp = LCA(a,c);
